...
const buttonVariants = cva(
  "inline-flex items-center justify-center whitespace-nowrap rounded-md text-sm font-medium ring-offset-background transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-2 disabled:pointer-events-none disabled:opacity-50 relative overflow-hidden",
  {
    variants: {
      variant: {
        default: "bg-primary text-primary-foreground hover:bg-primary/90 shadow-[0_0_20px_-5px_rgba(16,185,129,0.5)]",
        destructive:
        ...
        shiny: "bg-primary/10 backdrop-blur-md border border-primary/20 text-primary hover:bg-primary/20",
      },
      size: {
  ...
  ({ className, variant, size, children, ...props }, ref) => {
    return (
      ...
      >
        ...
        {variant === 'default' && (
             <span className="absolute inset-0 -translate-x-full group-hover:animate-[shimmer_1s_infinite] bg-linear-to-r from-transparent via-white/20 to-transparent z-0" />
        )}
      </motion.button>
    )
  }
